...
contract('Sbp', accounts => {
  ...
  it('should add events', async () => {
    ...
    const event1Receipt = await instance.addEvent.sendTransaction(
      'nba',
      'Charlotte',
      'OKC',
      32534524800
    );
    const event2Receipt = await instance.addEvent.sendTransaction(
      'nba',
      'Boston',
      'Milwaukee',
      32534611200
    ...
    assert.equal(event1[0], 'nba');
    assert.equal(event1[1], 'Charlotte');
    assert.equal(event1[2].toString(), '10000');
    assert.equal(event1[3], 'OKC');
    assert.equal(event1[4].toString(), '10000');
    assert.equal(event1[5].toString(), '32534524800');
    assert.equal(event1[6].toString(), '0');
    truffleAssert.eventEmitted(event1Receipt, 'NewEvent');

    assert.equal(event1[0], 'nba');
    assert.equal(event2[1], 'Boston');
    assert.equal(event2[2].toString(), '10000');
    assert.equal(event2[3], 'Milwaukee');
    assert.equal(event2[4].toString(), '10000');
    assert.equal(event2[5].toString(), '32534611200');
    assert.equal(event2[6].toString(), '0');
  ...
  it('should reject new event request from non-owner address', async () => {
    ...
    truffleAssert.fails(
      instance.addEvent.sendTransaction('nba', 'Charlotte', 'OKC', 32534524800, {
        from: accounts[1],
      })
    );
  });
  ...
  it('should get events', async () => {
    ...
    await instance.addEvent.sendTransaction('nba', 'Boston', 'Milwaukee', 32534611200);

    const events = await instance.getEvents();

    assert.equal(events.length, 2);

    assert.equal(events[0].eventType, 'nba');
    assert.equal(events[0].option1Name, 'Charlotte');
    assert.equal(events[0].option1PayoutOdds.toString(), '10000');
    assert.equal(events[0].option2Name, 'OKC');
    assert.equal(events[0].option2PayoutOdds.toString(), '10000');
    assert.equal(events[0].startTime.toString(), '32534524800');
    assert.equal(events[0].result.toString(), '0');

    assert.equal(events[1].option1Name, 'Boston');
    assert.equal(events[1].option1PayoutOdds.toString(), '10000');
    assert.equal(events[1].option2Name, 'Milwaukee');
    assert.equal(events[1].option2PayoutOdds.toString(), '10000');
  ...
  it('should get event by id', async () => {
    ...
    assert.equal(event1.eventType, 'nba');
    assert.equal(event1.option1Name, 'Charlotte');
    assert.equal(event1.option1PayoutOdds.toString(), '10000');
    assert.equal(event1.option2Name, 'OKC');
    assert.equal(event1.option2PayoutOdds.toString(), '10000');
    assert.equal(event1.startTime.toString(), '32534524800');
    assert.equal(event1.result.toString(), '0');

    assert.equal(event2.option1Name, 'Boston');
    assert.equal(event2.option2PayoutOdds.toString(), '10000');
    assert.equal(event2.option2Name, 'Milwaukee');
    assert.equal(event2.option2PayoutOdds.toString(), '10000');
  ...
  it('should get eligible betting events', async () => {
    ...
    await instance.addEvent.sendTransaction('nba', 'Charlotte', 'OKC', 32534524800);
    await instance.addEvent.sendTransaction('nba', 'Boston', 'Milwaukee', 666);
    await instance.addEvent.sendTransaction('nba', 'Toronto', 'Memphis', 888);

    const events = (await instance.getEligibleBettingEvents()).filter(
      event => !isEmptyEvent(event)
    );
  ...
  it('should reject new event result request from non-owner address', async () => {
    ...
    await instance.addEvent.sendTransaction('nba', 'Charlotte', 'OKC', 32534524800);
    truffleAssert.fails(instance.setEventResult.sendTransaction(0, 1, { from: accounts[1] }));
  });
  ...
  it('should reject bets placed after event has started', async () => {
    ...
    await instance.addEvent.sendTransaction('nba', 'Charlotte', 'OKC', 666);

    truffleAssert.fails(instance.placeBet.sendTransaction(0, 1));
  });
  ...
  it('should adjust event payout odds after bet is placed', async () => {
    ...
    const eventAfterBet1 = await instance.getEvent(0);

    console.log('event after bet 1');
    console.log(eventAfterBet1);
    ...
    const eventAfterBet2 = await instance.getEvent(0);

    console.log('event after bet 2');
    console.log(eventAfterBet2);
  });
  ...
  it('should get bets', async () => {
    ...
    assert.equal(bets[0].payoutOdds.toString(), '10000');
    ...
    assert.equal(bets[1].payoutOdds.toString(), '10000');
  });
  ...
  it('should get bet by id', async () => {
    ...
    assert.equal(bet1.payoutOdds.toString(), '10000');
    ...
    assert.equal(bet2.payoutOdds.toString(), '10000');
  });
  ...
  it('should get unclaimed bets', async () => {
    ...
    const bets = (await instance.getUnclaimedBets({ from: accounts[1] })).filter(
      bet => !isEmptyBet(bet)
    );
  ...
  it('should calculate bet payout amount', async () => {
    ...
    const betPayout = await instance.calculateBetPayoutAmount(bet, {
      from: accounts[1],
    });
  ...
  it('should reject bet payout claim if bet was lost', async () => {
    ...
    truffleAssert.fails(instance.claimBetPayout.sendTransaction(0, { from: accounts[1] }));
  });
  ...
  it('should reject bet payout claim if requested by foreign address', async () => {
    ...
    truffleAssert.fails(instance.claimBetPayout.sendTransaction(0, { from: accounts[2] }));
  });
});
